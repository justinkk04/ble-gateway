...
class PowerManager:
    ...
    POLL_INTERVAL = 4.0    # Seconds between poll cycles
    READ_STAGGER = 1.0     # Seconds between READ commands to nodes
    STALE_TIMEOUT = 15.0   # Seconds before marking node unresponsive
    COOLDOWN = 6.0         # Seconds between adjustments
    HEADROOM_MW = 500.0    # Target buffer below threshold (budget = threshold - headroom)
    STEP_SIZE = 5          # Max duty% change per adjustment cycle (prevents oscillation)
    ...
    async def _evaluate_and_adjust(self):
        ...
        # (means we haven't seen the effect of our last change yet)
        ...
        # Dead band: skip if within 2% of budget (prevents constant jitter)
        if abs(total_power - budget) < budget * 0.02:
            return

        # Also skip if all nodes are already at target_duty and under budget
        all_at_target = all(
    ...
    def _estimate_mw_per_pct(self, ns: NodeState, all_nodes: dict) -> float:
        """Estimate milliwatts per duty% for a node from live sensor data."""
        if ns.duty > 0 and ns.power > 0:
            return ns.power / ns.duty
        # Fallback: average from other nodes that have data
        estimates = [n.power / n.duty for n in all_nodes.values()
                     if n.duty > 0 and n.power > 0]
        if estimates:
    ...
    async def _nudge_node(self, nid: str, ns: NodeState, target_share_mw: float,
        ...
        if ideal_duty > current:
            new_duty = min(current + self.STEP_SIZE, int(ideal_duty))
        elif ideal_duty < current:
            new_duty = max(current - self.STEP_SIZE, int(ideal_duty))
        else:
            return None
