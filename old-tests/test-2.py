...
class PowerManager:
    ...
    POLL_INTERVAL = 4.0    # Seconds between poll cycles (was 2.0)
    READ_STAGGER = 1.0     # Seconds between READ commands to nodes (was 0.5)
    STALE_TIMEOUT = 15.0   # Seconds before marking node unresponsive (was 10.0)
    DAMPING = 0.9
    COOLDOWN = 6.0         # Seconds between adjustments (was 3.0)
    ...
    async def disable(self):
        ...
        for ns in self.nodes.values():
            if ns.responsive and ns.commanded_duty != ns.target_duty:
                self.gateway.log(
                ...
                await asyncio.sleep(1.5)  # Was 0.5 — mesh needs time between commands
    ...
    def on_sensor_data(self, node_id: str, duty: int, voltage: float,
        ...
        # Sync commanded_duty once sensor confirms the new duty
    ...
    async def _evaluate_and_adjust(self):
        ...
        # Skip if any node's sensor data is older than our last adjustment
        # (means we haven't seen the effect of our last reduction yet)
        if self._last_adjustment > 0:
            for ns in responsive.values():
                if ns.last_seen < self._last_adjustment:
                    return  # Still waiting for fresh data after last adjustment
    ...
    MIN_DUTY = 5  # Never reduce below 5% (prevents killing nodes)
    ...
    async def _reduce_proportional(self, nodes: dict, total_power: float):
        ...
        for nid, ns in nodes.items():
            # Use commanded_duty (what we last sent) not sensor duty (may be stale)
            base_duty = ns.commanded_duty if ns.commanded_duty > 0 else ns.duty
            new_duty = max(self.MIN_DUTY, int(base_duty * target_ratio))
            # Exception: if user explicitly stopped this node, allow 0
            if ns.target_duty == 0:
                new_duty = 0
            if new_duty != ns.commanded_duty:
                changes.append(f"N{nid}:{base_duty}->{new_duty}%")
                await self.gateway.set_duty(
    ...
    async def _reduce_with_priority(self, nodes: dict, total_power: float):
        ...
        if priority_ns.power >= self.threshold_mw:
            ...
            for nid, ns in non_priority.items():
                base_duty = ns.commanded_duty if ns.commanded_duty > 0 else ns.duty
                if base_duty > 0:
                    changes.append(f"N{nid}:{base_duty}->0%")
                    await self.gateway.set_duty(
                        nid, 0, _from_power_mgr=True, _silent=True)
                    ns.commanded_duty = 0
                    await asyncio.sleep(1.5)
            # Also reduce priority node
            pri_base = priority_ns.commanded_duty if priority_ns.commanded_duty > 0 else priority_ns.duty
            ratio = (self.threshold_mw / priority_ns.power) * self.DAMPING
            new_duty = max(self.MIN_DUTY, int(pri_base * ratio))
            changes.append(f"N{self.priority_node}:{pri_base}->{new_duty}%(pri)")
            await self.gateway.set_duty(
                self.priority_node, new_duty, _from_power_mgr=True, _silent=True)
            priority_ns.commanded_duty = new_duty
            self.gateway.log(
