...
class PowerManager:
    ...
    async def _evaluate_and_adjust(self):
        ...
        if not responsive:
            self.gateway.log("[PM] skip: no responsive nodes")
            return
        ...
        if diff < deadband:
            self.gateway.log(
                ...
                f"diff={diff:.0f} < band={deadband:.0f})")
            return

        # Skip if all nodes are at their ceiling and under budget
        # (nothing more to do — they can't go higher)
        all_at_ceiling = all(
            (ns.target_duty > 0 and ns.commanded_duty >= ns.target_duty)
            for ns in responsive.values()
        )
        if all_at_ceiling and total_power <= budget:
            self.gateway.log(
                f"[PM] skip: all at ceiling & under budget "
                f"(total={total_power:.0f} <= {budget:.0f})")
            return
    ...
    async def _nudge_node(self, nid: str, ns: NodeState, target_share_mw: float,
        ...
        new_duty = int(ideal_duty_clamped)
        ...
        await self.gateway._wait_node_response(nid)  # Wait for confirmation
        return change
